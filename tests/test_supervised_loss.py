...
def test_supervised_projection_matches_loss_and_gradients(reduction, masked, preselected):
    ...
    selected = (labels.reshape(-1) != -100).nonzero(as_tuple=True)[0] if preselected else None
    b = supervised_cross_entropy(other_h, other_w, labels, 7, reduction, selected=selected)
...
def test_checkpointed_tied_model_supervised_gradients(device, dtype):
    ...
    with torch.autocast(device, dtype=dtype):
        ...
        selected = (labels.cpu().reshape(-1) != -100).nonzero(as_tuple=True)[0].to(device)
        lb = b(x, labels=labels, return_logits=False, loss_reduction='sum',
               supervised_indices=selected)[1]
